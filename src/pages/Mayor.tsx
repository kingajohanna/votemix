...
import { initialMayor } from "../utils/data";
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      querySnapshot.forEach((doc) => {
        if (doc.id !== "admin" && doc.data().mayor?.length > 0)
          guesses.push({
            username: doc.id,
            data: doc.data().mayor.map((row: PersonData) => ({
              name: row.name,
              percentage: row.percentage,
            })),
          });
        else if (doc.id === "admin" && doc.data().mayor?.length > 0)
          setFinal({
            username: doc.id,
            data: doc.data().mayor.map((row: PersonData) => ({
              name: row.name,
